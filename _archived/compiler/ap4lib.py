...
class ActiveInstructionHeader:
    def __init__(self, opcode=0, goto=0):
        self.opcode = opcode
        self.goto = goto

    def getBytes(self):
        instr = ( bytes(chr(self.goto), 'utf-8') + bytes(chr(self.opcode), 'utf-8') )
        return instr
...
class ActiveProgram:
    def __init__(self, program):
        ...
        for i in range(0, len(program)):
            ...
            self.program.append(ActiveInstructionHeader(opcode=self.OPCODES[opcode], goto=label))
        self.program.reverse()
        self.program.append(ActiveInstructionHeader(opcode=self.OPCODES['EOF'], goto=0))
