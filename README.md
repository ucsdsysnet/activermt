...
```
git clone --depth 1 https://github.com/ucsdsysnet/activermt.git
...
```
4. Setup virtual interfaces.
```
sudo -E $SDE/install/bin/veth_setup.sh
```
5. Run the Tofino model.
```
cd $SDE
...
You will need to clone this repository on both the switch CPU (connected to the ASIC) as well as the server where you are running the (DPDK) applications from.

### Requirements
The following hardware and software requirements must be met:
1. A Tofino ASIC.
2. Intel SDK for Tofino (version 9.7.0).
3. A DPDK capable NIC (http://core.dpdk.org/supported/nics/) for running active network applications.
4. DPDK (https://doc.dpdk.org/guides/index.html). We have tested using version 21.11.5.
5. A Linux (x86) machine, preferrably running Ubuntu (focal 20.04).
...
```
cd <activermt_source_dir>
mkdir -p logs/controller
$SDE/run_bfshell.sh -b $(pwd)/activermt/controlplane/controller.py -i
```

The controller is now ready to admit new applications. 

The examples folder contains applications that use the active runtime. These applications are meant to be run on a server connected to the switch (asic). The servers need to be set up with DPDK. Before we evaluate stateful applications such as a key-value store, let's run a stateless application based on DPDK. Make sure that DPDK is installed and the environment variables set correctly.
