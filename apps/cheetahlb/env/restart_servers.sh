...
do
    if [ -z "$(lxc-info -n ap4-server-$SID | grep STOPPED)" ]
    then
        echo "stopping ap4-server-$SID..."
        lxc-stop -n ap4-server-$SID
    fi
done
...
do
    ...
    then
        ...
        lxc-attach -n ap4-server-$SID -- echo "export ACTIVEP4_SRC=/root/activep4" > /root/.bash_profile
    fi
done
