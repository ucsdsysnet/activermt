...
class Analysis:
    ...
    INSTR_SET_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'activermt', 'opcode_action_mapping.csv')
...
# run.
analysis.run(data_dir='evals/data', local=config['local'], config_name=os.path.basename(config_path).split('.')[0])