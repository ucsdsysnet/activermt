...
def runAnalysis(appCfg, metric, optimize, minimize, numRepeats, appname=None, w='random', departures=False, departureFID='random', departureProb=0.0, expId=0, debug=False, fixedSequence=None, seqLen=256, allowFilling=False):
    ...
    param_fit = 'ff' if not optimize else ('min' if minimize else 'max')
    param_constr = 'lc' if allowFilling else 'mc'
    outputDirName = "stats_g%d_n%d_%s_%s_%s_%d" % (Allocator.ALLOCATION_GRANULARITY, seqLen, param_workload, param_fit, param_constr, metric)
    if os.path.exists(os.path.join(os.getcwd(), outputDirName)):
        raise Exception("Stats directory {} already exists! Remove/rename existing directory.".format(outputDirName))
...
if custom:
    ...
    # metric = Allocator.METRIC_COST
    metric = Allocator.METRIC_REALLOC
    granularity = Allocator.ALLOCATION_GRANULARITY
    if ignoreIglim:
        for app in appCfg:
            appCfg[app]['iglim'] = -1
    print("Running analysis with parameters: optimize=%s, minimize=%s, workload=%s, granularity=%d, numapps=%d, constrained=%s, metric=%d" % (str(optimize), str(minimize), (appname if type == 'fixed' else type), Allocator.ALLOCATION_GRANULARITY, numApps, 'least' if allowFilling else 'most', metric))
